...
export function ControlesFiltrosGlobales({ filtros, usaAnios, onCambiar }: Props) {
  return (
    <section
      className="flex flex-wrap items-end gap-x-6 gap-y-3 rounded-lg border border-borde bg-panel px-4 py-3"
    ...
    >
      ...
      <fieldset className="min-w-0">
        <legend className="mb-1.5 inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wide text-apagado">
      ...
      <fieldset className="min-w-0" disabled={!usaAnios}>
        <legend className="mb-1.5 inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wide text-apagado">
          ...
          Rango de años
          {usaAnios ? null : (
            <span className="normal-case tracking-normal">· no aplica a este componente</span>
          )}
        </legend>
...
function BotonFenomeno({ activo, etiqueta, descripcion, color, onClick }: PropsBotonFenomeno) {
  return (
    <button
      type="button"
      aria-pressed={activo}
      ...
      className={cn(
        ...
        activo
          ? "border-acento/60 bg-acento/15 text-acento"
  ...
  return (
    <span className="inline-flex flex-col">
      ...
      <input
        ...
        className="h-8 w-20 rounded-md border border-borde bg-fondo px-2 font-mono text-xs text-texto disabled:opacity-50"
      />
    </span>
  );
}
