...
| `orden_observacion`      | `municipio` (nombre o código DIVIPOLA; vacío: el de más alertas), `economia`, `desde`, `hasta` | `{territorio:{divipola, municipio, departamento, divipola_dpto, centro:[lng,lat]\|null, area_km2, fuente_geometria}, alertas:{total, ultima_fecha, por_anio:[{anio, alertas, refs}], tipos, economias, grupos_armados, poblaciones, codigos, refs}, presencia_armada:{total_grupos, grupos, …, refs}\|null, mineria_detectada:{municipal, departamental:[{etiqueta, nuevo_ha, acumulado_ha}], en_cobertura, cobertura, procedencia}, menciones:{entidad, documentos, fragmentos, refs}\|null, candidatos:[…]}` — la ficha «de la mención al sobrevuelo»: las pasadas satelitales las propaga la SPA desde los TLE sobre `territorio.centro`; solo en el selector del tablero, por el catálogo congelado del agente |
| `evidencia_satelital`    | `sitio` (id ELDOR), `encuadre`: `frontera` \| `mineria` | `{sitios: [], triptico: {sitio, imagen, encuadre, con_anotacion, recorte_px, fecha_captura, resolucion_m_px, crs, modelo, huella_minera_ha, bosque_ha, clases: [{clase, porcentaje, minera, color}], procedencia}}` — medición sobre imagen: no lleva `doc_id`/`chunk_id`, su trazabilidad es espacial. Los trípticos los precalcula `scripts/eldor_recorte.py`; sin ellos, `triptico` es `null`. Solo en el selector del tablero |
