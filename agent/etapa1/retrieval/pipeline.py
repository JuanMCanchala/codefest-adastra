...
from ..schema import DocResult, FragmentResult, QueryResult
    ...
    def _doc_centroid(self, doc_id: str, scored_chunks: list) -> object | None:
        ...
        for cid, did, _score in scored_chunks:
            ...
            try:
                ...
                # IndexFlat reconstruye siempre; otros tipos de indice pueden no soportarlo.
                continue
    ...
    def _select_diverse_docs(self, ranked_docs, scored_chunks, n_docs: int) -> list[str]:
        ...
        for doc_id, _score in ranked_docs:
            ...
            if (
                c is not None
                and centroides
                and max(float(np.dot(c, o)) for o in centroides) >= umbral
            ):
