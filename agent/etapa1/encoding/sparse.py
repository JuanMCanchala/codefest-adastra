...
class SparseIndex:
    ...
    def add(self, lexical_weights: list[dict], chunk_ids: list[str]) -> None:
        ...
        if len(lexical_weights) != len(chunk_ids):
            raise ValueError("pesos y chunk_ids desalineados")
        acc: dict[str, list[tuple[int, float]]] = defaultdict(list)
        # conservar lo ya indexado
        for token, (idxs, ws) in self.inverted.items():
            acc[token] = list(zip(idxs.tolist(), ws.tolist(), strict=False))

        for weights, cid in zip(lexical_weights, chunk_ids, strict=False):
        ...
        self.inverted = {
            token: (
                np.fromiter((i for i, _ in pairs), dtype=np.int32, count=len(pairs)),
    ...
    def load(cls, in_dir: str | Path) -> SparseIndex | None:
        ...
        index.inverted = {
            token: (
                np.fromiter((int(i) for i, _ in entries), dtype=np.int32, count=len(entries)),
