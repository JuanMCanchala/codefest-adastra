...
| `meta-llama/Llama-Prompt-Guard-2-86M`                  | Licencia de Llama 4, con **acceso restringido y aprobación manual** en Hugging Face | Se evaluó a partir de su _model card_: es el único con evaluación publicada en español (AUC multilingüe de 0,995; _recall_ del 97,5 % con 1 % de falsos positivos en inglés). | **Fuera hasta que Meta apruebe el acceso.** No se puede empaquetar en la imagen sin los pesos. Si llega la aprobación, es el candidato para sustituir o complementar a proventra, con el umbral recalibrado sobre la misma batería. |
