...
> **Estado del código.** Todas las decisiones de esta tabla están implementadas y cableadas:
> `guard.evaluar` se invoca en `agent/app/graph.py` y `escaneo.sanear_fragmentos` en
> `agent/app/agents.py`, ambas con pruebas.
...
| S1  | Seguridad en **dos niveles**: rechazo duro solo ante credenciales y ejecución de código; aislamiento y neutralización para lo demás | La métrica cuenta como resistido el ataque cuya instrucción se ignora; bloquear cuesta falsos positivos         | Cableado en `graph.py` y `agents.py` |
| S2  | Escaneo de los fragmentos recuperados contra inyección indirecta                                                                    | El corpus es un canal de inyección indirecta (Greshake et al., 2023)                                            | Cableado en `graph.py` y `agents.py` |
| S3  | _Datamarking_ de los fragmentos, al estilo de _spotlighting_                                                                        | Bajó la tasa de éxito de los ataques de más del 50 % a menos del 2 % (Hines et al., 2024)                       | Cableado en `graph.py` y `agents.py` |
